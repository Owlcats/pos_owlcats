<div class="custom_tab_owlcats_outer">
    <div class="row">
        <div class="col-md-12 text-center">
            <h2 class="main_title">Master Roles</h2>
        </div>
...
<script type="text/javascript">
	...
	function list_data_roles(){
		$.ajax({
            type    : 'GET',
            url     : "<?=base_url('company_setting/list_data_roles');?>",
	...
	function add_list_data_roles(){
        ...
        $.ajax({
            type    : 'POST',
            url     : "<?=base_url('company_setting/form_roles');?>",
            data    : formData,
            timeout : 60000, // 60 detik timeout
            success : function (data) {
                obj = JSON.parse(data);
                if (obj.status == 'success') {
                    ...
                    $('#save_form').attr('onClick', 'save_form_roles();');
    ...
    function edit_list_data_roles(roles_code){
        var roles_code  = roles_code;
        var formData    = {
            'roles_code' : roles_code
        };
        $.ajax({
    ...
    function save_form_roles(){
        var roles_code  = document.getElementById('roles_code').value;
        var roles_name  = document.getElementById('roles_name').value;

        if (roles_name == '') {
            alert('Kolom nama roles tidak boleh kosong')
            return;
        }
        ...
        $.ajax({
            type    : 'POST',
            url     : "<?=base_url('company_setting/save_form_roles');?>",
            data    : formData,
            timeout : 60000, // 60 detik timeout
            success : function (data) {
                obj = JSON.parse(data);
    ...
    function activated_roles(roles_code){
        var roles_code          = roles_code;
        ...
        $.ajax({
            type    : 'POST',
            url     : "<?=base_url('company_setting/activated_roles');?>",
            data    : formData,
            timeout : 60000, // 60 detik timeout
            success : function (data) {
                obj = JSON.parse(data);
